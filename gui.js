...
GCodeViewer.Gui = function(domElement, callbacks) {
    ...
    addWidget("displayInIn", x, y, "data:image/png;base64," + GCodeViewer.inImage);
    addWidget("displayInMm", x, y, "data:image/png;base64," + GCodeViewer.mmImage);
    ...
    //Defining new variable because we can screw up callbacks outside this class

    //Planes display
    var showX = callbacks.showX;
    that.widgets.showX.onclick = function(){
        showX();
    };
    var showY = callbacks.showY;
    that.widgets.showY.onclick = function(){
        showY();
    };
    var showZ = callbacks.showZ;
    that.widgets.showZ.onclick = function(){
        showZ();
    };

    //Display
    var displayInIn = callbacks.displayInIn;
    that.widgets.displayInIn.onclick = function(){
        displayInIn();
        that.widgets.displayInIn.hidden = true;
        that.widgets.displayInMm.hidden = false;
    };
    var displayInMm = callbacks.displayInMm;
